...
Arguments
  --destination_file|-df   [Required]: Destination file for the kubeconfig
...
throw_if_empty --destination_file $destination_file
...
az acs kubernetes get-credentials --name "containerservice-$resource_group" --resource-group "$resource_group" --file "$destination_file" --ssh-key-file "$temp_key_path"
